...
for(set.dir.i in seq_along(set.dir.vec)){
  ...
  # Consider only the penalty learning data sets
  if(!(grepl("H3K36me3", set.name) || 
       grepl("H3K27ac", set.name) || 
       grepl("H3K4me3", set.name) ||
       grepl("blastoma", set.name))) next
  
  out.dir <- file.path("predictions", "IntervalRegressionCV.raw.features", set.name)
  ...
  if(file.exists(predictions.csv)){
    ...
    # Keep only raw (untransformed) features
    is.raw <- sapply(names(features), 
                     (function (x) !any(c(grepl("log.", x), 
                                          grepl("log+", x)))))
    features <- features[, is.raw, with=FALSE]
...
save(penaltyLearning.predictions, file="penaltyLearning.raw.features.predictions.RData")
