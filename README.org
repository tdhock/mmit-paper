...
- He emailed back, with this code to solve the problem
  ...
  positive (positive slope), and we need to partition only on
  intercept (not slope/variance). after redoing the test error figure,
  this does indeed fix the issue (trafotree is now learning as well as
