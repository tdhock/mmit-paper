...
- Simulated dataset add noise and remove examples, currently MMITs are
  as good as linear on linear dataset
...
- Worst case for the squared loss (empirical + theoretical) :
  Guillem's example of quadratic worst case.
...
- Figure showing validation error as a function of regularization
