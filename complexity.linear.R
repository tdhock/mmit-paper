...
for(test.fold in seq_along(fit.list)){
  vstats.list[[paste(set.name, test.fold)]] <- data.table(
    test.fold, fit.list[[test.fold]]$plot.heatmap.tile[margin==1])
}
vstats <- do.call(rbind, vstats.list)

vstats.min <- vstats[, {
  .SD[mean==min(mean)]
}, by=list(variable, test.fold)]
mse.dt <- vstats[variable=="mean.squared.error"]
vstats.selected <- vstats.min[, {
  v <- .SD[[variable]]
  dt <- data.table(test.fold, .SD)
  mse <- mse.dt[dt, mean, on=list(test.fold, regularization, margin)]
  .SD[order(v, mse)][1]
}, by=list(variable, test.fold)]
selected.tab <- vstats.selected[, table(variable, test.fold)]
